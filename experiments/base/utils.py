...
def prepare_logs(env_name: str, algo_name: str, argvs: List[str]):
    print(f"---- Train {algo_name} on {env_name} {time.strftime('%d-%m-%Y %H:%M:%S')} ----", flush=True)
    ...
    p["save_path"] = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), f"../{env_name}/exp_output/{p['experiment_name']}/{p['algo_name']}"
    )
