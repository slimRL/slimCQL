...
class PrioritizedSamplingTest(parameterized.TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.sampler = samplers.Prioritized(max_capacity=10)
    ...
    def test_sample(self):
        ...
        key_1, key_2, key_3 = jax.random.split(self.key, 3)

        # test if zero priority absent
        samples = self.sampler.sample(key_1, 5)
        ...
        samples = self.sampler.sample(key_2, 5)
        np.testing.assert_array_less(samples, 2)

        self.sampler.remove(0)
        samples = self.sampler.sample(key_3, 5)
